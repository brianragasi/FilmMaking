  ...
  async function runAction(name) {
    ...
    if (name === 'scrub') {
      ...
      setMetrics({
        ...
        blockedNote: 'malicious requests',
...
function bootAttackerConsole() {
  const terminal = document.querySelector('[data-attacker-terminal]');
  const commandInput = document.querySelector('[data-attacker-command-input]');
  const commandForm = document.querySelector('[data-attacker-command-form]');
  const resetButton = document.querySelector('[data-attacker-reset]');

  if (!terminal || !commandInput || !commandForm || !resetButton) {
    return;
  }

  const statusNode = document.querySelector('[data-attacker-status]');
  const deviceNode = document.querySelector('[data-attacker-devices]');
  const rateNode = document.querySelector('[data-attacker-rate]');
  const pressureNode = document.querySelector('[data-attacker-pressure]');
  const sentNode = document.querySelector('[data-attacker-sent]');
  const acceptedNode = document.querySelector('[data-attacker-accepted]');
  const rejectedNode = document.querySelector('[data-attacker-rejected]');
  const liveDot = document.querySelector('[data-attacker-live-dot]');
  const targetDot = document.querySelector('[data-attacker-target-dot]');
  const targetState = document.querySelector('[data-attacker-target-state]');
  ...
  function line(text, tone = 'info', prompt = '') {
    ...
    messageNode.className = `terminal-${tone}`;
  ...
  function setAttackerStatus(label, tone = 'slate') {
    ...
    statusNode.className = `rounded px-2 py-1 font-mono text-[9px] font-black ${tones[tone]}`;
  }

  function updateCounters() {
    rateNode.textContent = `${requestRate.toLocaleString()} / min`;
    sentNode.textContent = sent.toLocaleString();
    acceptedNode.textContent = accepted.toLocaleString();
    rejectedNode.textContent = rejected.toLocaleString();
    const pressure = Math.min(100, Math.round((requestRate / 68000) * 100));
    pressureNode.textContent = `${pressure}%`;
  }

  function drawAttackerBars(level = 0) {
    bars.forEach((bar, index) => {
      const spread = Math.round(Math.random() * 20);
      const rise = Math.min(98, 5 + level * 0.72 + spread + (index > 14 ? level * 0.12 : 0));
      bar.style.height = `${rise}%`;
    });
  }

  function setNodes(mode) {
    const counts = [12, 9, 15, 12];
    nodes.forEach((node, index) => {
      node.classList.remove('node-ready', 'node-active');
      if (mode === 'ready') {
        node.classList.add('node-ready');
      }
      if (mode === 'active') {
        node.classList.add('node-active');
      }
      node.querySelector('[data-attacker-node-count]').textContent = mode === 'offline' ? '0' : counts[index];
      node.querySelector('[data-attacker-node-state]').textContent = mode === 'active' ? 'Repeating' : mode === 'ready' ? 'Ready' : 'Offline';
    });
  }
  ...
  function primeInput() {
    ...
    executing = false;
    commandInput.value = '';
    const next = commandForStage();
    commandInput.disabled = !next;
    commandInput.placeholder = next ? 'ready' : stage === 'starting' ? 'requests increasing' : 'stopped';
    if (next) {
      setTimeout(() => commandInput.focus(), 0);
    }
  }
  ...
  async function beginRequests() {
    ...
    liveDot.className = 'h-2.5 w-2.5 rounded-full bg-rose-500 shadow-[0_0_14px_rgba(244,63,94,0.8)]';
    setNodes('active');
    ...
    activityTimer = setInterval(() => {
      requestRate = ramp[position];
      const batch = Math.round(requestRate / 60);
      sent += batch;
      accepted += Math.round(batch * 0.92);
  ...
  async function holdRequests() {
    ...
    activityTimer = setInterval(() => {
      ...
      const batch = Math.round(requestRate / 60);
      sent += batch;
      accepted += Math.round(batch * (filtering ? 0.14 : 0.88));
      rejected += Math.round(batch * (filtering ? 0.86 : 0.12));
      drawAttackerBars(filtering ? 42 : 100);
      updateCounters();
    }, 900);
    primeInput();
  }
  ...
  async function disconnectGroups() {
    ...
    stage = 'closed';
    requestRate = 0;
    setAttackerStatus('CLOSED', 'slate');
    deviceNode.textContent = '0';
    liveDot.className = 'h-2.5 w-2.5 rounded-full bg-slate-700';
    document.body.classList.remove('attacker-running');
    setNodes('offline');
    drawAttackerBars(0);
    updateCounters();
  ...
  async function executeCommand() {
    const scripted = commandForStage();
    if (!scripted || executing) {
      return;
    }
    executing = true;
    clearTimeout(commandTimer);
  ...
  function handleKey(event) {
    ...
    if (event.key === 'Backspace') {
      event.preventDefault();
      commandIndex = Math.max(0, commandIndex - 2);
      commandInput.value = scripted.text.slice(0, commandIndex);
      return;
    }
    if (event.key.length !== 1 && event.key !== 'Tab') {
      event.preventDefault();
    ...
    event.preventDefault();
    commandIndex = Math.min(scripted.text.length, commandIndex + (event.key === 'Tab' ? 8 : 1 + Math.floor(Math.random() * 3)));
    commandInput.value = scripted.text.slice(0, commandIndex);
    commandInput.setSelectionRange(commandInput.value.length, commandInput.value.length);
    if (commandIndex === scripted.text.length) {
      commandTimer = setTimeout(executeCommand, 240);
    }
  }
  ...
  function resetAttacker() {
    ...
    accepted = 0;
    rejected = 0;
    localStorage.setItem('ecocart_system_state', 'healthy');
    terminal.innerHTML = '';
    setAttackerStatus('STANDBY', 'green');
    deviceNode.textContent = '0';
    targetDot.className = 'h-2 w-2 rounded-full bg-slate-600';
    targetState.textContent = 'Not selected';
    targetState.className = 'font-mono text-[9px] font-black uppercase text-slate-500';
    liveDot.className = 'h-2.5 w-2.5 rounded-full bg-slate-700';
    document.body.classList.remove('attacker-running');
    setNodes('offline');
    drawAttackerBars(0);
  ...
  document.addEventListener('keydown', (event) => {
    ...
    event.preventDefault();
    commandInput.focus();
    commandInput.dispatchEvent(new KeyboardEvent('keydown', { key: event.key, bubbles: false }));
  });
  window.addEventListener('storage', (event) => {
    if (event.key !== 'ecocart_system_state' || stage === 'idle' || stage === 'closed') {
      return;
    }
  ...
  resetButton.addEventListener('click', resetAttacker);
  resetAttacker();
}
