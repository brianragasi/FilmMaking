...
<body class="min-h-screen bg-[#f4f5f7] text-slate-950" data-scene-client data-scene-view="storefront" data-scene-cue="<?= htmlspecialchars((string) $sceneState['cue']) ?>" data-scene-revision="<?= (int) $sceneState['revision'] ?>" data-scene-updated="<?= htmlspecialchars((string) $sceneState['updated_at']) ?>">
    ...
    <header class="border-b border-slate-200 bg-white">
        <div class="app-shell-narrow flex min-h-[72px] items-center gap-3">
            ...
            <?php if ($user): ?>
                <a class="btn btn-sm border-slate-200 bg-white" href="account.php"><i data-lucide="user-round" class="h-4 w-4"></i><span class="hidden sm:inline">Account</span></a>
            <?php else: ?>
                            ...
                            <?php foreach ($comments as $comment): ?>
                                ...
                                <li class="scroll-mt-5 rounded-lg border border-slate-200 bg-white p-4 sm:p-5" id="comment-<?= (int) $comment['id'] ?>">
                                    <article>
                                        ...
                                        <footer class="mt-4 flex flex-wrap items-center gap-2 border-t border-slate-100 pt-3"><span class="mr-1 text-[10px] font-black uppercase text-slate-400">React</span><?php foreach ($reactionTypes as $reactionKey => $reaction): ?><?php $reactionCount = (int) ($comment[$reactionKey . '_count'] ?? 0); $hasReacted = !empty($comment['viewer_reactions'][$reactionKey]); ?><?php if ($isCustomer): ?><form method="post" action="discussion-action.php"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars(csrf_token()) ?>"><input type="hidden" name="action" value="react"><input type="hidden" name="product_id" value="<?= $productId ?>"><input type="hidden" name="discussion_id" value="<?= (int) $comment['id'] ?>"><input type="hidden" name="reaction" value="<?= htmlspecialchars($reactionKey) ?>"><button class="btn btn-sm min-h-8 gap-1.5 rounded-full px-3 <?= $hasReacted ? 'border-rose-200 bg-rose-50 text-rose-700' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50' ?>" type="submit" aria-pressed="<?= $hasReacted ? 'true' : 'false' ?>"><i data-lucide="<?= htmlspecialchars($reaction['icon']) ?>" class="h-3.5 w-3.5 <?= $hasReacted && $reactionKey === 'love' ? 'fill-current' : '' ?>"></i><?= htmlspecialchars($reaction['label']) ?><?php if ($reactionCount > 0): ?><span class="font-black"><?= $reactionCount ?></span><?php endif; ?></button></form><?php else: ?><a class="btn btn-sm min-h-8 gap-1.5 rounded-full border-slate-200 bg-white px-3 text-slate-600" href="login.php?mode=login&amp;next=product.php" title="Sign in to react"><i data-lucide="<?= htmlspecialchars($reaction['icon']) ?>" class="h-3.5 w-3.5"></i><?= htmlspecialchars($reaction['label']) ?><?php if ($reactionCount > 0): ?><span class="font-black"><?= $reactionCount ?></span><?php endif; ?></a><?php endif; ?><?php endforeach; ?></footer>
                                    </article>
                                </li>
                            <?php endforeach; ?>
                        </ol>
